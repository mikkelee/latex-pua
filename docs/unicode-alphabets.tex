...
Where \verb|options| must be one or more of the following character sets. See references for further detail on each.
...
If no fonts are configured, the document font will be used (note that this may result in undesirable results, as few fonts support multiple character sets\footnote{I believe Andreas Stötzner's \emph{Andron Mega} does, albeit I haven't tried it as it is somewhat expensive.}).
